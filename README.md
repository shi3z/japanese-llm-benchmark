...
U字曲線がTurboQuant圧縮由来なのか検証するため、同一モデル・同一テストをFP16 KVキャッシュ（圧縮なし）で実施。

#### 比較結果

![TurboQuant vs FP16](turboquant_vs_fp16.png)

| サイズ | TurboQuant | FP16 | 差分 | 処理時間 (TQ/FP16) |
|--------|------------|------|------|-------------------|
| **150K** | **40%** | 0% | TQ +40% | 38s / 32s |
| **250K** | 20% | **40%** | FP16 +20% | 66s / 52s |
| **350K** | 40% | 40% | 同等 | 97s / 76s |

#### 重要な発見

1. **U字曲線はTurboQuant特有の現象**
   - FP16では単調増加パターン（0%→40%→40%）
   - TurboQuantでは250K付近でU字型の谷

2. **TurboQuantは小サイズで優位**
   - 150KではFP16が全滅（0%）に対し、TurboQuant40%成功
   - KV圧縮が注意機構に独自のパターンを形成している可能性

3. **250K付近がTurboQuantの弱点**
   - FP16（40%）に対しTurboQuant（20%）
   - 圧縮バッファ境界での情報損失と推測

4. **処理速度はTurboQuantが20-30%遅い**
   - 圧縮/解凍オーバーヘッドの影響
   - ただしVRAM使用量は大幅削減（5x圧縮）
...
**実用的な推奨事項**:
- **TurboQuant推奨**: VRAM制限がある場合、または100K以下/400K以上のコンテキスト
- **FP16推奨**: VRAM十分、かつ150K-300K付近のコンテキストを多用する場合
