...
| 🆕 | **Bonsai-8B** | 良好* | - | - | 383 tok/s | 1.2GB |

*Bonsai-8Bは関連キーワードを豊富に抽出（例: CNN, RNN → +違い, ニューラルネットワーク）。1.2GBで超軽量。

**注意**: Qwopus3.5-9Bは思考モード(thinking)が常時有効のため、キーワード抽出タスクには不向き。要約タスクに最適化。
