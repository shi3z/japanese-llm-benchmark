...
- **テスト数**: 12回（サイズ・位置をランダム化）

### 結果サマリー

| Test | サイズ | 針の位置 | トークン数 | 処理時間 | 結果 |
|------|--------|----------|------------|----------|------|
| 1 | 100K | 10% | 50,740 | 11.0s | ✅ PASS |
| 2 | 100K | 50% | 51,195 | 10.6s | ✅ PASS |
| 3 | 100K | 90% | 50,839 | 10.4s | ❌ FAIL |
| 4 | 200K | 20% | 101,638 | 22.0s | ❌ FAIL |
| 5 | 200K | 50% | 101,898 | 22.1s | ✅ PASS |
| 6 | 200K | 80% | 101,935 | 22.2s | ❌ FAIL |
| 7 | 300K | 30% | 152,096 | 36.9s | ❌ FAIL |
| 8 | 300K | 70% | 152,420 | 37.2s | ❌ FAIL |
| 9 | 400K | 25% | 203,302 | 55.9s | ❌ FAIL |
| 10 | 400K | 75% | 202,493 | 55.7s | ✅ PASS |
| 11 | 500K | 40% | 254,218 | 79.2s | ✅ PASS |
| 12 | 500K | 60% | 254,226 | 78.5s | ✅ PASS |
...
| コンテキストサイズ | 成功率 | 備考 |
|-------------------|--------|------|
| 100K (~50K tokens) | 67% (2/3) | 末尾配置で失敗 |
| 200K (~100K tokens) | 33% (1/3) | 中央配置のみ成功 |
| 300K (~150K tokens) | 0% (0/2) | 全て失敗 |
| 400K (~200K tokens) | 50% (1/2) | 後半配置で成功 |
| 500K (~250K tokens) | 100% (2/2) | 全て成功 |

### 定性的評価 - 成功例

#### Test 5 (200K, 50%位置)
- **秘密コード**: `EPSILON_TEST_200K_50`
- **周辺コンテキスト**: プログラミングの民主化、AIが人間の知性を超える議論
- **モデル出力**: Document 20 内の `[重要な秘密情報]` マーカーを正確に発見し、コードを抽出

```
The prompt explicitly states: 以下は大量のドキュメントの集合です...
And at the end of Document 20, there is a specific marker:
[重要な秘密情報] このドキュメントには秘密のコードが含まれています: EPSILON_TEST_200K_50
```

#### Test 11 (500K, 40%位置)
- **秘密コード**: `LAMBDA_TEST_500K_40`
- **周辺コンテキスト**: 情報伝達の進化、Markdown、AIの応答フォーマット
- **トークン数**: 254,218
- **処理時間**: 79.2秒
- **結果**: ✅ 正確に発見
...
#### Test 3 (100K, 90%位置)
- **秘密コード**: `GAMMA_TEST_100K_90`
- **周辺コンテキスト**: AI生成コンテンツのパーソナライズ化
- **失敗理由**: ドキュメント末尾(90%)に配置された針をモデルが見落とし

#### Test 7 (300K, 30%位置)
- **秘密コード**: `ETA_TEST_300K_30`
- **周辺コンテキスト**: Microsoft Bob (1995年)、エージェントの歴史
- **トークン数**: 152,096
- **失敗理由**: 150K tokens を超えるとコンテキスト序盤の情報も見落とす傾向

### 考察

1. **500K (250K tokens) で高い成功率**: 意外にも最大サイズで全問正解。モデルがより広いコンテキストに適応している可能性

2. **300K (150K tokens) で最低成功率**: 中間サイズでの性能低下が顕著。コンテキスト長とモデルの注意機構のミスマッチか

3. **位置依存性**: 末尾配置(80-90%)での失敗率が高い。「Lost in the Middle」現象と一致

4. **処理速度**: 254K tokens でも約80秒で処理完了。TurboQuant による圧縮が有効
...
| **成功率** | 50% (12回中6回) |
| **推奨コンテキストサイズ** | 100K tokens以下 (67%成功) |
| **注意点** | 末尾配置の情報は見落としやすい |
| **処理速度** | 250K tokens で ~80秒 |

**実用的な長コンテキスト利用**: TurboQuant により物理的にはコンテキストを拡張可能だが、モデルの注意機構の限界により、重要情報はコンテキストの中央付近に配置することを推奨。
